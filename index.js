...
(function initEmailJS() {
  if (typeof emailjs !== 'undefined' && EMAILJS_PUBLIC_KEY && EMAILJS_PUBLIC_KEY !== "YOUR_EMAILJS_PUBLIC_KEY") {
    ...
    console.error("EmailJS SDK script library is not loaded. Ensure the CDN script tag is in the <head> of your index.html.");
  } else {
    console.warn("EmailJS loaded, but is waiting for you to populate your active Public Key in voltex.js.");
  }
})();
...
async function handleEnquiry(e) {
  ...
  const enquiry_type = document.getElementById('eq-type').value || 'General Enquiry'
  ...
  try {
    ...
    if (typeof emailjs !== 'undefined' && EMAILJS_PUBLIC_KEY && EMAILJS_PUBLIC_KEY !== "YOUR_EMAILJS_PUBLIC_KEY") {
      ...
      console.warn("EmailJS call bypassed. Populate your Public Key at the top of voltex.js to enable email dispatches.");
    }
...
function renderProductsTable() {
  ...
  if (!paginated.length) {
    tbody.innerHTML = '<tr><td colspan="6" style="color:var(--muted);text-align:center;padding:2rem">No products found matching filters.</td></tr>'
    document.getElementById('prod-pagination-info').textContent = 'Showing 0 to 0 of 0 entries'
  ...
  if (controlsEl) {
    controlsEl.innerHTML = buildTruncatedPaginationHTML(productCurrentPage, totalPages, 'changeProductPage')
  }
}
...
async function saveProduct() {
  ...
  try {
    ...
    renderOverview()
    showCustomAlert(editingProductId ? 'Product updated successfully.' : 'Product created successfully.', 'success')
  } catch (e) {
...
async function deleteProduct(id) {
  const confirmed = await showCustomConfirm('Delete this product? This cannot be undone.')
  if (!confirmed) return
  try {
...
function renderEnquiriesTable() {
  ...
  if (!paginated.length) {
    tbody.innerHTML = '<tr><td colspan="10" style="color:var(--muted);text-align:center;padding:2rem">No enquiries found matching criteria.</td></tr>'
    document.getElementById('enq-pagination-info').textContent = 'Showing 0 to 0 of 0 entries'
  ...
  if (controlsEl) {
    controlsEl.innerHTML = buildTruncatedPaginationHTML(enquiryCurrentPage, totalPages, 'changeEnquiryPage')
  }
}
...
function openProductMobileFilters() {
  document.getElementById('prod-search-input-mob').value = document.getElementById('prod-search-input').value;
  document.getElementById('prod-limit-select-mob').value = document.getElementById('prod-limit-select').value;
...
function openEnquiryMobileFilters() {
  document.getElementById('enq-search-input-mob').value = document.getElementById('enq-search-input').value;
  document.getElementById('enq-limit-select-mob').value = document.getElementById('enq-limit-select').value;
  document.getElementById('filter-status-mob').value = document.getElementById('filter-status').value;
  document.getElementById('filter-type-mob').value = document.getElementById('filter-type').value;
...
document.getElementById('catalogue-lead-modal')?.addEventListener('hidden.bs.modal', () => {
...
function openCatalogueLeadModal(productName) {
  ;['cat-lead-name', 'cat-lead-company', 'cat-lead-phone', 'cat-lead-email'].forEach(id => {
  ...
  if (subtitleEl) subtitleEl.textContent = productName ? `For: ${productName}` : 'Please share your details to continue'
...
async function logCatalogueDownload(lead, productId, productName) {
  ...
  if (typeof emailjs !== 'undefined' && EMAILJS_PUBLIC_KEY && EMAILJS_PUBLIC_KEY !== "YOUR_EMAILJS_PUBLIC_KEY") {
    ...
    if (EMAILJS_ADMIN_TEMPLATE_ID) {
      emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_ADMIN_TEMPLATE_ID, adminParams)
        .then(res => console.log("Catalogue-download admin notification triggered:", res.status, res.text))
        .catch(err => console.error("Catalogue-download admin EmailJS trigger failed:", err))
    }
    if (EMAILJS_CLIENT_TEMPLATE_ID && EMAILJS_CLIENT_TEMPLATE_ID !== "YOUR_CLIENT_TEMPLATE_ID") {
      emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_CLIENT_TEMPLATE_ID, clientParams)
        .then(res => console.log("Catalogue-download client auto-reply triggered:", res.status, res.text))
        .catch(err => console.error("Catalogue-download client EmailJS auto-reply failed:", err))
    }
  }
}
...
loadLandingProducts()
restoreSession().then(() => {
  if (isLoggedIn && document.getElementById('page-dashboard')?.classList.contains('active')) {
    showPage('dashboard')
  }
})